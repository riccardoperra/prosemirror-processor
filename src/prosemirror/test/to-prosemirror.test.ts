...
import {
  ...
  toProseMirrorMark,
  toProseMirrorNode,
} from "../to-prosemirror.js";
import { schema } from "prosemirror-test-builder";
import { unistNodeFromMarkdown } from "../../unified/index.js";
import type { Text } from "mdast";

const defaultHandlers: HastNodeHandlers = {
  root(node, _, context) {
  ...
  text(node) {
    ...
    return schema.text(String(_node.value ?? ""));
  },
};

describe("remark-prosemirror", async () => {
  it("should process a doc with paragraphs", async () => {
    const handlers: HastNodeHandlers = {
      ...defaultHandlers,
