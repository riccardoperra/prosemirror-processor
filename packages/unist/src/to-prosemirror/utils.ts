...
import type { ToProseMirrorNodeHandler } from "./context.js";
...
export function toProseMirrorNode<TNode extends Unist.Node>(
...
): ToProseMirrorNodeHandler<TNode> {
  return (node, _, context) => {
    ...
    return pmNode(
      type,
      context.handleAll(node as unknown as Unist.Parent),
      getAttrs?.(node) ?? null,
      {
        mode: "fill",
      },
...
export function toProseMirrorMark<TNode extends Unist.Node>(
  ...
  return (node, _, context) => {
    ...
    const children = context.handleAll(node as unknown as Unist.Parent);
